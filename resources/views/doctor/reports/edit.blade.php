                    ...
                    <div>
                        ...
                        <span class="badge ms-2 text-capitalize
                            {{ $report['status'] === 'published' ? 'bg-success' :
                               ($report['status'] === 'submitted' ? 'bg-warning text-dark' :
                               ($report['status'] === 'revision_requested' ? 'bg-danger' : 'bg-secondary')) }}">
                                    ...
                                    <div class="doc-checkbox-card p-3 rounded-3 border {{ !empty($docs[$key]) ? 'checked' : '' }}"
                                        ...
                                        <div class="form-check mb-0 d-flex align-items-center gap-2">
                                            <input class="form-check-input doc-check" type="checkbox"
                                                name="documents_reviewed[{{ $key }}]" value="1"
                                                id="doc_{{ $key }}" {{ !empty($docs[$key]) ? 'checked' : '' }}
                                                {{ $editable ? '' : 'disabled' }}>
                                            <label class="form-check-label fw-medium" for="doc_{{ $key }}" style="cursor:inherit;">
                                                {{ $label }}
                                            </label>
                                        </div>
                                    </div>
                            ...
                            <div id="findings-list">
                                ...
                                <div class="finding-row d-flex gap-2 mb-2">
                                    <input type="text" class="form-control" name="key_findings[]"
                                        value="{{ $finding }}" placeholder="Finding {{ $i + 1 }}"
                                        {{ $editable ? '' : 'readonly' }}>
                                    @if($editable)
                                    <button type="button" class="btn btn-outline-danger btn-sm remove-finding flex-shrink-0">
                                    ...
                                    <div class="rec-checkbox-card p-3 rounded-3 border {{ !empty($recs[$key]) ? 'checked' : '' }}"
                                        ...
                                        <div class="form-check mb-0 d-flex align-items-center gap-2">
                                            <input class="form-check-input rec-check" type="checkbox"
                                                name="recommendations[{{ $key }}]" value="1"
                                                id="rec_{{ $key }}" {{ !empty($recs[$key]) ? 'checked' : '' }}
                                                {{ $editable ? '' : 'disabled' }}>
                                            <label class="form-check-label fw-medium" for="rec_{{ $key }}" style="cursor:inherit;">
                                                {{ $label }}
                                            </label>
                                        </div>
                                    </div>
                        ...
                        <div class="card-body p-4">
                            ...
                            <div id="questions-list">
                                ...
                                <div class="question-row d-flex gap-2 mb-2 align-items-center">
                                    <span class="text-muted small fw-semibold flex-shrink-0" style="width:20px;">{{ $i + 1 }}.</span>
                                    <input type="text" class="form-control" name="questions_for_physician[]"
                                        value="{{ $question }}" placeholder="Question {{ $i + 1 }}"
                                        {{ $editable ? '' : 'readonly' }}>
                                    @if($editable)
                                    <button type="button" class="btn btn-outline-danger btn-sm remove-question flex-shrink-0">
                                        <i class="isax isax-minus-cirlce"></i>
                                    </button>
                                    @endif
                                </div>
                                ...
                                @if(empty($questions))
                                <div class="question-row d-flex gap-2 mb-2 align-items-center">
                                    <span class="text-muted small fw-semibold flex-shrink-0" style="width:20px;">1.</span>
                                    <input type="text" class="form-control" name="questions_for_physician[]"
                                        placeholder="Question 1" {{ $editable ? '' : 'readonly' }}>
                                    @if($editable)
                                    <button type="button" class="btn btn-outline-danger btn-sm remove-question flex-shrink-0">
                                        <i class="isax isax-minus-cirlce"></i>
                                    </button>
                    ...
                    <div class="report-section card border-0 shadow-sm mb-4" id="patient-summary">
                        <div class="card-body p-4">
                            ...
                            @if($editable)
                                ...
                                <input type="hidden" name="patient_friendly_summary" id="input-patient-summary">
                            @else
                        ...
                        <div class="card border-0 shadow-sm sticky-top" style="top:80px;">
                            <div class="card-body p-3">
                                ...
                                <ul class="list-unstyled mb-0 stepper-nav" id="stepperNav">
                                    ...
                                    <li>
                                        <a href="#{{ $s['id'] }}" class="stepper-link d-flex align-items-center gap-2 py-2 px-2 rounded-3 text-decoration-none {{ $i === 0 ? 'active' : '' }}"
                                           data-section="{{ $s['id'] }}">
                                            <span class="stepper-num d-flex align-items-center justify-content-center flex-shrink-0"
                                                  style="width:26px;height:26px;border-radius:50%;background:#f1f5f9;font-size:12px;font-weight:700;color:#64748b;">
                                                {{ $i + 1 }}
                                            </span>
                                            <span style="font-size:13px;font-weight:500;color:#374151;">{{ $s['label'] }}</span>
                                        </a>
                                    </li>
                                    @endforeach
                                </ul>
...
{{-- Submit Form (hidden) --}}
<form id="submitForm" method="POST" action="{{ route('doctor.reports.submit', $report['id']) }}" style="display:none;">
    @csrf
</form>

{{-- Auto-save indicator --}}
<div id="saveIndicator" style="display:none;position:fixed;bottom:24px;right:24px;z-index:9999;">
    <div class="d-flex align-items-center gap-2 px-3 py-2 rounded-3 shadow" style="background:#fff;border:1px solid #e5e7eb;">
...
<style>
    ...
    .stepper-link { transition: background .15s; }
    .stepper-link:hover, .stepper-link.active {
        background: #eef2ff;
    }
    .stepper-link.active .stepper-num {
        background: #2563eb !important; color: #fff !important;
    }
    .stepper-link.active span:last-child { color: #2563eb !important; }

    .doc-checkbox-card, .rec-checkbox-card {
        transition: border-color .15s, background .15s;
    }
    .doc-checkbox-card.checked, .rec-checkbox-card.checked {
        border-color: #2563eb !important; background: #eef2ff;
    }
    .doc-checkbox-card:hover, .rec-checkbox-card:hover {
        border-color: #93c5fd;
    }

    .report-text-view { line-height: 1.7; color: #374151; }
    .report-text-view p { margin-bottom: 8px; }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initQuill(editorId, inputId) {
        ...
        q.on('text-change', () => { if (input) input.value = q.root.innerHTML; });
    ...
    document.querySelectorAll('.doc-check, .rec-check').forEach(cb => {
        cb.addEventListener('change', () => {
            cb.closest('.doc-checkbox-card, .rec-checkbox-card').classList.toggle('checked', cb.checked);
        });
    });
    ...
    document.getElementById('addFinding')?.addEventListener('click', () => {
        ...
        row.innerHTML = `<input type="text" class="form-control" name="key_findings[]" placeholder="Finding ${count}">
    ...
    document.getElementById('addQuestion')?.addEventListener('click', () => {
        ...
        row.innerHTML = `<span class="text-muted small fw-semibold flex-shrink-0" style="width:20px;">${count}.</span>
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const link = document.querySelector(`.stepper-link[data-section="${entry.target.id}"]`);
    ...
    function collectData() {
        ...
        data.documents_reviewed = {
            medical_records: false, laboratory_results: false, imaging_studies: false,
            pathology_reports: false, operative_reports: false, consultation_notes: false, other: ''
        };
        for (const [k, v] of fd.entries()) {
        ...
        data.recommendations = {
            additional_testing: false, additional_imaging: false, specialist_referral: false,
            treatment_modification: false, monitoring: false, surgical_consultation: false,
            lifestyle_modifications: false, other: false, details: ''
        };
        for (const [k, v] of fd.entries()) {
    ...
    async function saveDraft() {
        ...
        try {
            const res = await fetch('{{ route('doctor.reports.save-draft', $report['id']) }}', {
                method: 'POST',
                headers: {

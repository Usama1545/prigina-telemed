                                    ...
                                    <ul class="user-list">
                                        @forelse (($conversations ?? []) as $index => $conversation)
                                            <li class="user-list-item"
                                                ...
                                                data-receiver-id="{{ $conversation['doctorId'] ?? '' }}"
                                                data-receiver-name="{{ $conversation['doctorName'] ?? 'Doctor' }}">
                                                <a href="javascript:void(0);" class="conversation-item">
                                                    @php
                                                        $nameParts = explode(
                                                            ' ',
                                                            trim($conversation['doctorName'] ?? ''),
                                                        );
                                                            ...
                                                            <div class="last-chat-time">
                                                                ...
                                                                <div class="chat-pin">
                                                                    @if (($conversation['patientUnreadCount'] ?? 0) > 0)
                                                                        <span
                                                                            class="unread badge badge-primary">{{ $conversation['patientUnreadCount'] ?? 0 }}</span>
    ...
    <script>
        ...
        `);
            ...
            function openConversation(conversationId, doctorName = null) {
                ...
                currentReceiverName = li.data('receiver-name') || doctorName || 'Doctor';
            ...
            function renderCallCard(call) {
                ...
                // Patient view: own calls use the patient icon (green, right); received use doctor icon (blue, left)
                const avatarIcon = isCaller ? 'fa-user' : 'fa-user-doctor';

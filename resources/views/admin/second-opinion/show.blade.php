    ...
    <div class="content container-fluid" style="max-width:1100px;">
            ...
            <div class="row align-items-center">
                <div class="col">
                    ...
                    <ul class="breadcrumb mt-1">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('admin.second-opinion.index') }}">Second Opinion</a></li>
                ...
                <div class="col-auto d-flex gap-2">
                    @php
                        ...
                        [$statusColor, $statusLabel] = $statusMap[$report['status'] ?? 'draft'] ?? ['secondary', $report['status']];
                    @endphp
                    <span class="badge bg-{{ $statusColor }}-subtle text-{{ $statusColor }} px-3 py-2" style="font-size:13px;">
                        {{ $statusLabel }}
                    </span>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-3">
                <div class="card border-0 shadow-sm mb-3 sticky-top" style="top:76px;">
                    <div class="card-body p-3">
                        ...
                        <p class="fw-semibold small text-uppercase text-muted mb-3" style="letter-spacing:.05em;">Actions</p>

                        @if($report['status'] === 'submitted')
                        <form method="POST" action="{{ route('admin.second-opinion.approve', $report['id']) }}" class="mb-2">
                            @csrf
                            <button type="submit" class="btn btn-success w-100" onclick="return confirm('Approve this report?')">
                                <i class="fe fe-check-circle me-2"></i> Approve Report
                            </button>
                        </form>
                        <button type="button" class="btn btn-outline-danger w-100 mb-2" data-bs-toggle="modal" data-bs-target="#revisionModal">
                            <i class="fe fe-alert-circle me-2"></i> Request Revision
                        </button>
                        @endif

                        @if($report['status'] === 'approved')
                        <form method="POST" action="{{ route('admin.second-opinion.publish', $report['id']) }}" class="mb-2">
                            @csrf
                            <button type="submit" class="btn btn-primary w-100" onclick="return confirm('Publish this report to the patient?')">
                                <i class="fe fe-send me-2"></i> Publish to Patient
                            </button>
                        </form>
                        @endif

                        <a href="{{ route('doctor.reports.pdf', $report['id']) }}" target="_blank" class="btn btn-outline-secondary w-100">
                            <i class="fe fe-file-text me-2"></i> View PDF
                        </a>

                        <hr class="my-3">

                        <p class="fw-semibold small text-uppercase text-muted mb-2" style="letter-spacing:.05em;">Summary</p>
                        <div class="small">
                        ...
                        <hr class="my-3">
                        <p class="fw-semibold small text-uppercase text-muted mb-2" style="letter-spacing:.05em;">Revision Note</p>
            ...
            <div class="col-lg-9">
                ...
                @php
                    $docLabels = ['medical_records' => 'Medical Records', 'laboratory_results' => 'Lab Results', 'imaging_studies' => 'Imaging Studies', 'pathology_reports' => 'Pathology Reports', 'operative_reports' => 'Operative Reports', 'consultation_notes' => 'Consultation Notes'];
                @endphp
                <div class="report-card card border-0 shadow-sm mb-4">
                    <div class="card-header-section">
                    ...
                    <div class="card-body p-4">
                        <div class="row g-2">
                            ...
                            <div class="col-6 col-md-4">
                                <div class="doc-check-display p-2 rounded-3 border d-flex align-items-center gap-2
                                    {{ !empty($docs[$key]) ? 'checked' : '' }}">
                                    <span class="doc-check-icon flex-shrink-0">
                                        @if(!empty($docs[$key]))
                                            <i class="fe fe-check-square" style="color:#1d4ed8;font-size:16px;"></i>
                                        @else
                                            <i class="fe fe-square" style="color:#cbd5e1;font-size:16px;"></i>
                                        @endif
                                    </span>
                                    <span class="small fw-medium" style="color:{{ !empty($docs[$key]) ? '#1d4ed8' : '#94a3b8' }};">{{ $label }}</span>
                                </div>
                            </div>
                            @endforeach
                        </div>
                                ...
                                @foreach($findings as $i => $f)
                                    ...
                                    <div class="col-12">
                                        <div class="finding-item d-flex align-items-center gap-2 p-2 rounded-3"
                                            ...
                                            <span class="finding-num flex-shrink-0" style="width:22px;height:22px;border-radius:50%;background:#1d4ed8;color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;">
                                                {{ $i + 1 }}
                                            </span>
                                            <span class="small fw-medium" style="color:#374151;">{{ $f }}</span>
                                        </div>
                                    </div>
                                    @endif
                                @endforeach
                ...
                {{-- 8. Recommendations --}}
                @php
                    $recLabels = ['additional_testing' => 'Additional Testing', 'additional_imaging' => 'Additional Imaging', 'specialist_referral' => 'Specialist Referral', 'treatment_modification' => 'Treatment Modification', 'monitoring' => 'Monitoring', 'surgical_consultation' => 'Surgical Consultation', 'lifestyle_modifications' => 'Lifestyle Modifications', 'other' => 'Other'];
                @endphp
                <div class="report-card card border-0 shadow-sm mb-4">
                    <div class="card-header-section">
                        ...
                        <div class="row g-2 mb-3">
                            ...
                            <div class="col-6 col-md-4">
                                <div class="doc-check-display p-2 rounded-3 border d-flex align-items-center gap-2
                                    ...
                                    <span class="doc-check-icon flex-shrink-0">
                                        @if(!empty($recs[$key]))
                                            <i class="fe fe-check-square" style="color:#16a34a;font-size:16px;"></i>
                                        @else
                                            <i class="fe fe-square" style="color:#cbd5e1;font-size:16px;"></i>
                                        @endif
                                    </span>
                                    <span class="small fw-medium" style="color:{{ !empty($recs[$key]) ? '#15803d' : '#94a3b8' }};">{{ $label }}</span>
                                </div>
                            </div>
                            @endforeach
                        </div>
                                ...
                                @foreach($questions as $i => $q)
                                    ...
                                    <div class="col-12">
                                        <div class="finding-item d-flex align-items-start gap-2 p-2 rounded-3"
                                             style="background:#f8fafc;border:1px solid #e2e8f0;">
                                            <span class="finding-num flex-shrink-0 mt-1" style="width:22px;height:22px;border-radius:50%;background:#6366f1;color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;">
                                                {{ $i + 1 }}
                                            </span>
                                            <span class="small" style="color:#374151;line-height:1.5;">{{ $q }}</span>
                                        </div>
                                    </div>
                                    @endif
                                @endforeach
                ...
                <div class="report-card card border-0 shadow-sm mb-4">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="d-flex align-items-center gap-2 p-3 rounded-3"
                            ...
                            <div>
                                <p class="mb-0 fw-semibold small" style="color:#15803d;">Certified by physician</p>
                        ...
                        <div class="d-flex align-items-center gap-2 p-3 rounded-3"
                            ...
                            <p class="mb-0 small" style="color:#92400e;">Certification pending — report not yet submitted.</p>
                        </div>
                        @endif
                    </div>
                </div>
...
<style>
    .report-card { border-radius: 14px !important; overflow: hidden; }
    ...
    .card-header-section h6 { font-size: 14px; font-weight: 700; color: #1e293b; }
    ...
    .field-label { font-size: 11px; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: .04em; margin-bottom: 4px; }
    ...
    .field-input:focus { box-shadow: none !important; }
    ...
    .rich-text-display p { margin-bottom: 8px; }
    .rich-text-display p:last-child { margin-bottom: 0; }
    .rich-text-display em { color: #94a3b8; }
</style>
